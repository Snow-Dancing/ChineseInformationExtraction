...
def train():
    ...
    # make_path(FLAGS)  这多写了一遍

    log_path = os.path.join("log", FLAGS.log_file)
    logger = get_logger(log_path)   # 这是啥意思
    print_config(config, logger)  # 这是啥意思
...
def evaluate_test(read_path, save_path):
    config = load_config(FLAGS.config_file)
    logger = get_logger(FLAGS.log_file)
    # limit GPU memory
    tf_config = tf.ConfigProto()
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for i, line in enumerate(lines):
            print("\r当前进度:{:.2%}".format(i/length), end = '')
            result = model.evaluate_line(sess, input_from_line(line, char_to_id), id_to_tag)
            # print(result)
...
def main(_):
    ...
    if FLAGS.train:
        ...
        evaluate_line()
        # read_path = './pred/valid_data.txt'
        # save_path = './pred/valid_pre_results.json'
        # evaluate_test(read_path, save_path)
