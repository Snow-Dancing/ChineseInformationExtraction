...
def test_line():
    ...
    pathname = "./model/ATT_GRU_model-176790"
...
def test_file():
    ...
    test_path = './test/valid_pre_results.json'
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            count_test = 0
            count_relation = 0
            for num, test_data in enumerate(database):
                    print("\r当前进度是:{:.2%}".format(num/total_test), end = "")
                    #try:
                    #BUG: Encoding error if user input directly from command line.
                    #Read file from test file
                    '''
                    ...
                    with open(save_path, 'a', encoding='utf8') as f:
                        json.dump({"text":sentence, "spo_list":spo_list}, f, ensure_ascii=False)
                        f.write('\n')
                    #break
            print("\n找到关系的句子共:%d句" % count_test)
            print("一共找到:%d个关系" % count_relation)
                    #break
